...
% maximum_modes=400;  % Selection of maximum amount of modes
% interval = 100;      % Selection of interval
% numberOfModes=zeros(maximum_modes/interval,1);
% total_error=numberOfModes;
% mean_rel_error=numberOfModes;
% modeSequence = [0 10 20 40 50 55 60 65 110 120 125 130 140 150 160 170 200 210 220 230 240 250 260 270 280 290 300 320 340 360 380 400 420 440];
% 
% absStressErrorX = zeros(length(1:length(modeSequence)),1);
...
% 
% j=1;
% % for i = 0:interval:maximum_modes
% for i = [0 10 20 40 50 55 60 65 110 120 125 130 140 150 160 170 200 210 220 230 240 250 260 270 280 290 300 320 340 360 380 400 420 440]
%     numberOfModes(j)=i;
% 
% 
%     [t,U_dyn_cms]=DynamicCMSFEM(K,M,D,NodeGrid,i,[]);
%     [~,~,total_error(j),mean_rel_error(j)]=ErrorCalculation(t_dir,U_dyn_dir,t,U_dyn_cms);
% 
%     sigX = PhiX*U_dyn_cms;
%     sigY = PhiY*U_dyn_cms;
