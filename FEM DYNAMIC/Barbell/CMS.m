...
function [M_tilde_ii,D_tilde_ii,K_tilde_ii,M_tilde_ie,D_tilde_ie,K_tilde_ie,V_cms] = CMS(K,M,D,NumberOfModes, AdditionalModes, NodeGrid)
    ...
    [~,forceNodes] = ForceBoundaryCondition(NodeGrid,0);
    externalNodes = [boundNodes forceNodes];
    % Reconfiguration of matrices
    % [ A_ee A_ei;
    %   A_ie A_ii]
    [K, K_ee, K_ei, K_ie, K_ii]= MatrixReconfiguration(K, externalNodes);
    [M, M_ee, M_ei, M_ie, M_ii]= MatrixReconfiguration(M, externalNodes);
    [D, D_ee, D_ei, D_ie, D_ii]= MatrixReconfiguration(D, externalNodes);


    % Eigenmodes of the free system
    if NumberOfModes == 0
        V_cms = [eye(length(externalNodes)); -K_ii\K_ie];
    else
        [Phi, Omega] = eigs(K_ii,M_ii,NumberOfModes,'smallestabs');
        Phi = [Phi, AdditionalModes];
        
        V_cms = [eye(length(externalNodes)) zeros(length(externalNodes),length(Phi(1,:)));
                 -K_ii\K_ie Phi];
    end
